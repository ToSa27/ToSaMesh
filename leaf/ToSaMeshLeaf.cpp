...
bool ToSaMeshLeaf::isBankHoliday() {
...
void ToSaMeshLeaf::resetFlags() {
	for (uint8_t i = 0; i < MEM_FLAG_COUNT; i++)
		changeFlag[i] = 0;
}

bool ToSaMeshLeaf::isFlagged(uint8_t address) {
	return ((changeFlag[address >> 3] & (1 << (address & 0x03))) > 0);
}

uint16_t ToSaMeshLeaf::getValue(uint8_t address) {
...
void ToSaMeshLeaf::setValue(uint8_t address, uint16_t value, uint8_t setFlag) {
...
void ToSaMeshLeaf::rxConfig(appMessage am) {
	...
	if ((configType & TOSA_MESH_CONFIG_FETCHALL) > 0) {
		appMessage amr;
		amr.source = node->addr;
		amr.dest = nodeConfig.rootAddress;
		amr.pid = 0;
		amr.ptype = TOSA_MESH_PTYPE_CONFIG_REQUEST;
		amr.len = 2;
		...
		for (;;) {
			...
			if (doFetch) {
				amr.data[0] |= TOSA_MESH_CONFIG_FETCHALL;
				node->Send(amr);
				return;
			} else {
				amr.data[0] += 1;
				amr.data[1] = 0;
				if (amr.data[0] > TOSA_MESH_CONFIG_TYPE_LAST) {
					saveConfig();
					return;
				}
			}
...
void ToSaMeshLeaf::organizeConfigMem() {
	uint8_t* configPos = configData;
	ioConfig = (IOConfig *)configPos;
	configPos += configHeader.ioCount * TOSA_MESH_CONFIG_TYPE_IO_SIZE;
	networkConfig = (NetworkConfig *)configPos;
	configPos += configHeader.networkCount * TOSA_MESH_CONFIG_TYPE_NETWORK_SIZE;
	controlConfig = (ControlConfig *)configPos;
	configPos += configHeader.controlCount * TOSA_MESH_CONFIG_TYPE_CONTROL_SIZE;
	parameterConfig = (ParameterConfig *)configPos;
	configPos += configHeader.parameterCount * TOSA_MESH_CONFIG_TYPE_PARAMETER_SIZE;
	timerConfig = (TimerConfig *)configPos;
	configPos += configHeader.timerCount * TOSA_MESH_CONFIG_TYPE_TIMER_SIZE;
}

void ToSaMeshLeaf::fetchConfig() {
	validConfig = false;
	appMessage amReq;
	amReq.source = node->addr;
	amReq.dest = nodeConfig.rootAddress;
	amReq.pid = 0;
	amReq.ptype = TOSA_MESH_PTYPE_CONFIG_REQUEST;
	amReq.len = 2;
	amReq.data[0] = TOSA_MESH_CONFIG_TYPE_HEADER | TOSA_MESH_CONFIG_FETCHALL;
	amReq.data[1] = 0;
	node->Send(amReq);
}

uint16_t ToSaMeshLeaf::getConfigSize() {
	uint16_t res = 0;
	res += configHeader.ioCount * TOSA_MESH_CONFIG_TYPE_IO_SIZE;
	res += configHeader.networkCount * TOSA_MESH_CONFIG_TYPE_NETWORK_SIZE;
	res += configHeader.controlCount * TOSA_MESH_CONFIG_TYPE_CONTROL_SIZE;
	res += configHeader.parameterCount * TOSA_MESH_CONFIG_TYPE_PARAMETER_SIZE;
	res += configHeader.timerCount * TOSA_MESH_CONFIG_TYPE_TIMER_SIZE;
	return res;
}

void ToSaMeshLeaf::saveConfig() {
...
uint8_t ToSaMeshLeaf::onewire_reset(uint8_t pin) {
	cli();
...
void ToSaMeshLeaf::onewire_writebit(uint8_t pin, uint8_t data) {
	cli();
	digitalWrite(pin, LOW);
	pinMode(pin, OUTPUT);
...
void ToSaMeshLeaf::onewire_writebyte(uint8_t pin, uint8_t data) {
...
uint8_t ToSaMeshLeaf::onewire_readbit(uint8_t pin) {
	cli();
	pinMode(pin, OUTPUT);
	digitalWrite(pin, LOW);
	delayMicroseconds(3);
...
uint8_t ToSaMeshLeaf::onewire_readbyte(uint8_t pin) {
    uint8_t data = 0;
    for (uint8_t bitmask = 0x01; bitmask; bitmask <<= 1)
...
void ToSaMeshLeaf::execTimer(uint8_t timer) {
...
void ToSaMeshLeaf::rxNetwork() {
	while (node->HasData()) {
		appMessage am = node->Receive();
		switch (am.ptype) {
			case TOSA_MESH_PTYPE_ANNOUNCE_RESPONSE:
				if (1 == 1) {
				...
				continue;
			case TOSA_MESH_PTYPE_CONFIG_RESPONSE:
				rxConfig(am);
				continue;
			case TOSA_MESH_PTYPE_DATASYNC:
				for (uint8_t i = 0; i < configHeader.networkCount; i++) {
					...
					if (proto != PROTO_NETWORK_NONE)
						if ((proto & 0x80) > 0) // rx
							if ((am.source == networkConfig[i].address) || (networkConfig[i].address == TOSA_MESH_BROADCAST))
				...
				continue;
			case TOSA_MESH_PTYPE_DATA_SET:
				setValue(am.data[0], am.data[1] + (am.data[2] >> 8), 2);
				continue;
			case TOSA_MESH_PTYPE_DATA_GET_REQUEST:
				if (1 == 1) {
					appMessage amr;
					amr.source = node->addr;
					amr.dest = am.source;
					amr.pid = 0;
					amr.ptype = TOSA_MESH_PTYPE_DATA_GET_RESPONSE;
					amr.len = 3;
			...
			case TOSA_MESH_PTYPE_CONFIG_REQUEST:
				if (1 == 1) {
					...
					amr.dest = am.source;
					amr.pid = 0;
					amr.ptype = TOSA_MESH_PTYPE_CONFIG_RESPONSE;
					uint8_t* datasrc;
					uint8_t datalen;
					switch (am.data[0]) {
						case TOSA_MESH_CONFIG_TYPE_HEADER:
...
void ToSaMeshLeaf::execControls() {
...
uint8_t ToSaMeshLeaf::getParam(uint8_t type, ParameterConfig * pc, uint8_t count) {
...
void ToSaMeshLeaf::execControl_OnOff(uint8_t ci) {
...
void ToSaMeshLeaf::execControl_Heating(uint8_t ci) {
	ControlConfig cc = controlConfig[ci];
	ParameterConfig * pc = &(parameterConfig[cc.paramStart]);
	// get current status
...
void ToSaMeshLeaf::execControl_Blind(uint8_t ci) {
	ControlConfig cc = controlConfig[ci];
	ParameterConfig * pc = &(parameterConfig[cc.paramStart]);
	// get current status
				...
				switch (proto) {
					case PROTO_NETWORK_TX:
						if (isFlagged(networkConfig[i].sourceMemAddress)) {
							...
							am.pid = 0;
							am.ptype = TOSA_MESH_PTYPE_DATASYNC;
							am.len = 4;
...
/*
void ToSaMeshLeaf::sendDebug(uint8_t position, uint8_t data1, uint8_t data2) {
	meshMessage mm;
	mm.tc = 255;
	mm.to = 0x0001;
	mm.from = node->addr;
	...
	mm.dest = 0x0001;
	mm.pid = (node->nextpid)++;
	mm.ptype = TOSA_MESH_PTYPE_DEBUG;
	mm.cost = 1;
	mm.len = 3;
	mm.data[0] = position;
	mm.data[1] = data1;
