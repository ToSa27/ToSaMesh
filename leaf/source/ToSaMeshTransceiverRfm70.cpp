...
ToSaMeshTransceiverRfm70::ToSaMeshTransceiverRfm70(uint8_t sclk, uint8_t mosi, uint8_t miso, uint8_t csn, uint8_t ce)
	: rfm70(*(new pins::uc_pin(sclk)), *(new pins::uc_pin(mosi)), *(new pins::uc_pin(miso)), *(new pins::uc_pin(csn)), *(new pins::uc_pin(ce))) {
}

bool ToSaMeshTransceiverRfm70::Init(uint16_t Address) {
	pins::uc_init();
	rfm70::init();

	SetAddress(Address, false);
	SetAddress(TOSA_MESH_BROADCAST, true);
    rfm70::air_data_rate(2);
    rfm70::channel(TOSA_MESH_RFM70_CHANNEL);
...
void ToSaMeshTransceiverRfm70::SetAddress(uint16_t Address, bool brdcst) {
	uint8_t addrarr[5];
...
void ToSaMeshTransceiverRfm70::Send(meshMessage mm) {
...
bool ToSaMeshTransceiverRfm70::HasData() {
    return (!rfm70::receive_fifo_empty());
}

meshMessage ToSaMeshTransceiverRfm70::Receive() {
	rawMessageRfm70 rm;
...
meshMessage ToSaMeshTransceiverRfm70::RawToMesh(rawMessageRfm70 rm) {
	meshMessage mm;
...
rawMessageRfm70 ToSaMeshTransceiverRfm70::MeshToRaw(meshMessage mm) {
	rawMessageRfm70 rm;
	rm.to = mm.to;
