...
void ToSaMeshNode::TickTx() {
	for (uint16_t i = 0; i < txb->capacity; i++) {
		if (txb->pending(i)) {
			...
			if (fa != TOSA_MESH_NOADDRESS) {
				for (int rti = 0; rti < rt->capacity; rti++) {
					if ((rt->routes[rti].dest != TOSA_MESH_NOADDRESS) && (rt->routes[rti].hop == rti)) {
						LostRoute(rti);
					}
				}
			}
...
//		LOG4CXX_INFO(logger, log);
			...
			switch (mm.ptype) {
				...
				case TOSA_MESH_PTYPE_TIME_BROADCAST:
					...
					Forward(mm, true, false);
					continue;
				case TOSA_MESH_PTYPE_DISCOVER_REQUEST:
					if ((mm.dest == addr) || (mm.dest == TOSA_MESH_BROADCAST)) {
						for (uint8_t i = 0; i < rt->capacity; i++) {
							if (rt->routes[i].dest != TOSA_MESH_NOADDRESS) {
								meshMessage mmr;
								mmr.tc = mm.tc;
								mmr.to = mm.from;
								mmr.from = addr;
								...
								mmr.dest = mm.source;
								mmr.pid = nextpid++;
								mmr.ptype = TOSA_MESH_PTYPE_DISCOVER_RESPONSE;
								mmr.cost = 1;
								mmr.len = 6;
					...
					if (mm.dest != addr)
						Forward(mm, (mm.dest == TOSA_MESH_BROADCAST), false);
					continue;
