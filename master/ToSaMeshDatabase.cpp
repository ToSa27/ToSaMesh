...
DBClientConnection ToSaMeshDatabase::c;
map<uint8_t,uint8_t> ToSaMeshDatabase::memPrototype;

LoggerPtr ToSaMeshDatabase::logger(Logger::getLogger("ToSa.Mesh"));

void ToSaMeshDatabase::init() {
	c.connect("localhost:8017");
	if (c.count("ToSaMeshDb.Settings") < 1) {
		BSONObj s = BSON( "key" << "NextAvailableAddress" << "value" << 2 );
		c.insert("ToSaMeshDb.Settings", s);
...
std::string ToSaMeshDatabase::getSetting(std::string key) {
	std::auto_ptr<DBClientCursor> cursor = c.query("ToSaMeshDb.Settings", QUERY("key" << key));
	if (!cursor.get())
		return "";
	if (cursor->more())	{
...
void ToSaMeshDatabase::checkFirmwareDirectory() {
...
void ToSaMeshDatabase::loadFirmwareFile(std::string fwHexFile) {
	...
	vector<std::string> subs = ToSaMeshUtils::split(fwHexFilename, '_');
	if (subs.size() < 3)
		return;
...
uint8_t * ToSaMeshDatabase::getNewMac() {
	uint8_t mac[6];
	mac[0] =  84; // 0x54 - T
	mac[1] = 111; // 0x6F - o
	mac[2] =  83; // 0x53 - S
	...
	for(;;) {
		macs = macBtoS(mac);
		if (c.count("ToSaMeshDb.Node", BSON("mac" << macs)) == 0)
			break;
		if (mac[5] < 255)
			mac[5] += 1;
		else {
	...
	c.insert("ToSaMeshDb.Node", s);
	return macStoB(macs);
}

std::string ToSaMeshDatabase::macBtoS(uint8_t *in) {
	std::string res = "";
	for (uint8_t i = 0; i < 6; i++) {
		if (i > 0)
			res += ":";
		res += ToSaMeshUtils::u8tohex(in[i]);
	}
	return res;
}

uint8_t * ToSaMeshDatabase::macStoB(std::string in) {
	uint8_t res[6];
	for (uint8_t i = 0; i < 6; i++)
		res[i] = ToSaMeshUtils::hextou8(in.substr(3 * i, 2));
	return res;
}

uint16_t ToSaMeshDatabase::getAddress(uint8_t * mac) {
	std::string macs = macBtoS(mac);
	uint16_t addr = TOSA_MESH_NOADDRESS;
	std::auto_ptr<DBClientCursor> cursor = c.query("ToSaMeshDb.Node", QUERY("mac" << macs));
	...
	if (addr == TOSA_MESH_NOADDRESS) {
		...
		if (macExists) {
			c.update("ToSaMeshDb.Node", QUERY("mac" << macs), BSON("$set" << BSON("address" << addr)));
		} else {
...
uint16_t ToSaMeshDatabase::getNextAvailableAddress() {
...
bool ToSaMeshDatabase::nodeExists(uint16_t address) {
	return (c.count("ToSaMeshDb.Node", BSON("address" << address)) > 0);
}

void ToSaMeshDatabase::editNodeDetails(uint16_t address, uint16_t hwType, uint16_t hwVersion, uint16_t fwVersion, uint16_t fwCrc) {
	c.update("ToSaMeshDb.Node", QUERY("address" << address), BSON("$set" << BSON("hwType" << hwType << "hwVersion" << hwVersion << "fwVersion" << fwVersion << "fwCrc" << fwCrc)));
}

vector<uint16_t> ToSaMeshDatabase::getNodes() {
	vector<uint16_t> res;
	std::auto_ptr<DBClientCursor> cursor;
	cursor = c.query("ToSaMeshDb.Node");
	if (cursor.get())
		while (cursor->more()) {
...
bool ToSaMeshDatabase::nodeAutoUpdate(uint16_t address) {
	std::auto_ptr<DBClientCursor> cursor;
	cursor = c.query("ToSaMeshDb.Node", QUERY("address" << address));
	if (cursor.get())
		if (cursor->more()) {
			BSONObj b = cursor->next();
...
bool ToSaMeshDatabase::needsUpdate(uint16_t address) {
	std::auto_ptr<DBClientCursor> cursor;
	cursor = c.query("ToSaMeshDb.Node", QUERY("address" << address));
	if (cursor.get())
		if (cursor->more()) {
...
Firmware ToSaMeshDatabase::getLatestFirmware(uint16_t address) {
	Firmware fw;
	std::auto_ptr<DBClientCursor> cursor;
	cursor = c.query("ToSaMeshDb.Node", QUERY("address" << address));
	if (cursor.get())
...
Firmware ToSaMeshDatabase::getLatestFirmware(uint16_t hwType, uint16_t hwVersion) {
...
void ToSaMeshDatabase::getFirmwareBlock(uint16_t hwType, uint16_t hwVersion, uint16_t fwVersion, uint16_t fwBlockIndex, uint8_t data[]) {
...
uint8_t ToSaMeshDatabase::getNodeConfig(uint16_t address, uint8_t configType, uint8_t configId, uint8_t data[]) {
	data[0] = configType;
	data[1] = configId;
	uint8_t maskedConfigType = configType & 0x7F; //!TOSA_MESH_CONFIG_FETCHALL;
	std::auto_ptr<DBClientCursor> cursor;
	cursor = c.query("ToSaMeshDb.Node", QUERY("address" << address));
	if (cursor.get()) {
		if (cursor->more()) {
			...
			if (maskedConfigType == TOSA_MESH_CONFIG_TYPE_HEADER) {
			...
			switch (maskedConfigType) {
				case TOSA_MESH_CONFIG_TYPE_IO:
					boc = boConfig.getObjectField("io");
					break;
				case TOSA_MESH_CONFIG_TYPE_NETWORK:
					boc = boConfig.getObjectField("network");
					break;
				case TOSA_MESH_CONFIG_TYPE_CONTROL:
					boc = boConfig.getObjectField("control");
					break;
				case TOSA_MESH_CONFIG_TYPE_PARAMETER:
					boc = boConfig.getObjectField("parameter");
					break;
				case TOSA_MESH_CONFIG_TYPE_TIMER:
...
uint16_t ToSaMeshDatabase::getConfigCrcHelper(BSONObj boc, uint16_t crc) {
	uint16_t res = crc;
...
uint16_t ToSaMeshDatabase::getConfigCrc(uint16_t address) {
	std::auto_ptr<DBClientCursor> cursor;
	cursor = c.query("ToSaMeshDb.Node", QUERY("address" << address));
	if (cursor.get()) {
		if (cursor->more()) {
...
BSONObj ToSaMeshDatabase::getFirmware(uint16_t hwType, uint16_t hwVersion, uint16_t fwVersion) {
	std::auto_ptr<DBClientCursor> cursor;
	if (fwVersion == 0)
		cursor = c.query("ToSaMeshDb.Firmware", QUERY("hwType" << hwType << "hwVersion" << hwVersion).sort("fwVersion", -1));
	else
		cursor = c.query("ToSaMeshDb.Firmware", QUERY("hwType" << hwType << "hwVersion" << hwVersion << "fwVersion" << fwVersion));
	if (cursor.get())
		if (cursor->more())
...
void ToSaMeshDatabase::addFirmware(uint16_t hwType, uint16_t hwVersion, uint16_t fwVersion, std::string hexFile) {
	...
	if (!hexFile.empty()) {
		...
		for (std::string line; getline(hf, line);) {
			uint8_t reclen = ToSaMeshUtils::hextou8(line.substr(1, 2));
			uint16_t offset = ToSaMeshUtils::hextou16(line.substr(3, 4));
			uint8_t rectype = ToSaMeshUtils::hextou8(line.substr(7, 2));
			std::string data = line.substr(9, 2 * reclen);
			uint8_t chksum = ToSaMeshUtils::hextou8(line.substr(9 + (2 * reclen), 2));
			if (rectype == 0) {
				if ((start == 0) && (end == 0)) {
					if (offset % 128 > 0)
						return;
				...
				for (uint8_t i = 0; i < reclen; i++)
					bytes.push_back(ToSaMeshUtils::hextou8(data.substr(i * 2, 2)));
				end += reclen;
			}
		}
		...
		if (boOldFw.isEmpty())
			...
			newFw = true;
		if (newFw) {
			c.remove("ToSaMeshDb.Firmware", BSON("hwType" << hwType << "hwVersion" << hwVersion << "fwVersion" << fwVersion));
			BSONObj boFw = BSON("hwType" << hwType << "hwVersion" << hwVersion << "fwVersion" << fwVersion << "crc" << crc << "blocks" << babBlocks.arr() << "hexFile" << hexFile);
			c.insert("ToSaMeshDb.Firmware", boFw);
...
uint16_t ToSaMeshDatabase::crc16_update(uint16_t crc, uint8_t a) {
	int i;
	crc ^= a;
	for (i = 0; i < 8; ++i) {
		if (crc & 1)
...
void ToSaMeshDatabase::addDataLogging(uint8_t address, uint16_t raw) {
	if (memPrototype[address] == NULL) {
		...
		std::auto_ptr<DBClientCursor> cursor;
		cursor = c.query("ToSaMeshDb.Configuration", QUERY("key" << "DataLogger"));
		if (cursor.get()) {
			if (cursor->more()) {
...
//			bo = BSON("address" << address << "rawdata" << BSON_ARRAY(rawdataHigh << rawdataLow) << "data" << s);
	...
	c.insert("ToSaMeshDb.DataLogger", bo);
}

vector<string> ToSaMeshDatabase::getDataLogging(uint8_t address) {
	vector<string> res;
	std::auto_ptr<DBClientCursor> cursor = c.query("ToSaMeshDb.DataLogger", QUERY("address" << address));
	if (!cursor.get())
		return res;
	while (cursor->more())	{
		BSONObj p = cursor->next();
